"""Frozen research v1 CLI (Kalshi/NBM/dataset/backtest/live).

Split from the thin production entry (weadge.cli) so that heavy deps
(polars, duckdb, ...) are imported per-command, never at startup.
FROZEN: no new strategies in this lane.

    weadge kalshi sync-series KXHIGHNY
    weadge dataset build --series KXHIGHNY
...
# registered from weadge.cli (thin entry)
